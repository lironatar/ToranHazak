...
const ScheduleTimeline = ({
    ...
    // Persistent State: Load from localStorage only if date matches today
    const [collapsedLevels, setCollapsedLevels] = useState(() => {
        try {
            const today = new Date().toDateString();
            const savedState = localStorage.getItem('mission_collapsed_state');
            if (savedState) {
    ...
    const toggleLevel = (levelId) => {
        ...
        if (newSet.has(levelId)) newSet.delete(levelId); // Remove from set = Expand
        else newSet.add(levelId); // Add to set = Collapse
        setCollapsedLevels(newSet);

        // Save to LocalStorage with today's date
        try {
            const today = new Date().toDateString();
            localStorage.setItem('mission_collapsed_state', JSON.stringify({
                date: today,
    ...
    return (
        <div style={{ position: 'relative', paddingRight: '12px' }}>
            ...
            {schedule.map((level) => {
                const isLevelCollapsed = collapsedLevels.has(level.id);
                ...
                return (
                    <div key={level.id} style={{ position: 'relative', paddingRight: '20px', marginBottom: '48px' }}>
                        ...
                        {/* Level Header - Clickable to Toggle */}
                        <div
                        ...
                        >
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                ...
                                <motion.div
                                    animate={{ rotate: isLevelCollapsed ? 180 : 0 }}
                        ...
                        {/* MISSIONS CONTAINER with Animation */}
